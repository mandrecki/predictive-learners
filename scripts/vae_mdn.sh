...
ENV_NAMES=( "ball_in_cup-catch" "finger-spin" "cheetah-run" "CarRacing-v0" "Pong-ple-v0" "PixelCopter-ple-v0" "Catcher-ple-v0" ) # "TetrisA-v2" )
...
do
    ...
    do
        ...
        python ../pred_learn/train/train_wm_vae.py --env-id "${ENV_NAMES[i]}" --model-path $EXP_FOLDER/trained_models/predictive/"${ENV_NAMES[i]}"-1.pt --vis
        python ../pred_learn/train/train_wm_preds.py --env-id "${ENV_NAMES[i]}" --model-path $EXP_FOLDER/trained_models/predictive/"${ENV_NAMES[i]}"-1.pt --vis
