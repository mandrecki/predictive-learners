...
do
    ...
    do
        ...
        python ../pred_learn/train/train_wm_vae.py --env-id "${ENV_NAMES[i]}" --model-path $EXP_FOLDER/trained_models/predictive/"${ENV_NAMES[i]}"-1.pt --vis --n-epochs 30 --bit-depth $BIT_DEPTH # --extra-image
        python ../pred_learn/train/train_wm_preds.py --env-id "${ENV_NAMES[i]}" --model-path $EXP_FOLDER/trained_models/predictive/"${ENV_NAMES[i]}"-1.pt --vis --n-epochs 30 --bit-depth $BIT_DEPTH # --extra-image
