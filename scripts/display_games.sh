...
ENV_NAMES=(   "reacher-easy"  "ball_in_cup-catch" "finger-spin" "cheetah-run" "cartpole-balance" )
...
do
    ...
    do
        ...
        python ../pred_learn/data/experience_collector.py --env-id "${ENV_NAMES[i]}" --file-appendix "image-$j" --total-steps $STEPS --rl-model-path $EXP_FOLDER/trained_models/test/ppo/"${ENV_NAMES[i]}"-1.pt --recurrent-policy --extra-video --render --no-record --n-envs 1
