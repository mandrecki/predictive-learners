...
#ENV_NAMES=( "Catcher-ple-v0" ) # "TetrisA-v2" )
...
#        python ../pred_learn/train/train_planet.py --env-id "${ENV_NAMES[i]}" --model-path $EXP_FOLDER/trained_models/predictive/"${ENV_NAMES[i]}"-base-1.pt --vis --n-epochs 30 --bit-depth $BIT_DEPTH
#        python ../pred_learn/train/train_planet.py --env-id "${ENV_NAMES[i]}" --model-path $EXP_FOLDER/trained_models/predictive/"${ENV_NAMES[i]}"-image-1.pt --vis --n-epochs 30 --bit-depth $BIT_DEPTH --extra-image
        python ../pred_learn/train/train_planet.py --env-id "${ENV_NAMES[i]}" --model-path $EXP_FOLDER/trained_models/predictive/"${ENV_NAMES[i]}"-video-1.pt --vis --n-epochs 30 --bit-depth $BIT_DEPTH --extra-video
