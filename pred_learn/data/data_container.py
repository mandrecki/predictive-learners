...
class ObservationSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(idx, idx+self.series_length):
            ...
            sample["s0"].append(torch.FloatTensor(timestep["s0"]/255).permute([2, 1, 0]))
            sample["s1"].append(torch.FloatTensor(timestep["s1"]/255).permute([2, 1, 0]))
            sample["a0"].append(torch.LongTensor([timestep["a0"]]))
